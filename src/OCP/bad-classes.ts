...
class ShapeCalculator {
    ...
    // 💡 ПРОБЛЕМА: Длинная цепочка if-else с проверкой типов
    ...
    calculateArea(shapeType: string, radius?: number, width?: number, height?: number): number {
        if (shapeType === 'circle') {
            ...
            // 💡 ПРОБЛЕМА: Это означает, что класс НЕ ЗАКРЫТ для изменения
    ...
    calculatePerimeter(shapeType: string, radius?: number, width?: number, height?: number): number {
        if (shapeType === 'circle') {
            ...
            // 💡 ПРОБЛЕМА: Проблема повторяется в каждом методе класса
    ...
    // 🎯 КАК ИСПРАВИТЬ: Использовать интерфейс Shape и полиморфизм
