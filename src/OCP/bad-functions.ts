...
// 🎯 КАК ИСПРАВИТЬ: Использовать полиморфизм и интерфейсы
function calculateArea(shapeType: string, radius?: number, width?: number, height?: number): number {
    ...
    // 💡 ПРОБЛЕМА: Нужно знать все возможные типы фигур
    ...
    if (shapeType === 'circle') {
        ...
        // 💡 ПРОБЛЕМА: Это означает, что функция НЕ ЗАКРЫТА для изменения
        // 🎯 РЕШЕНИЕ: Triangle должен быть отдельным объектом с методом getArea()
        ...
        // 💡 ПРОБЛЕМА: Каждая новая фигура требует изменения существующего кода
        // 🎯 РЕШЕНИЕ: Square должен быть отдельным объектом с методом getArea()
...
// 💡 ПРОБЛЕМА: Дублирование логики проверки типов
...
function calculatePerimeter(shapeType: string, radius?: number, width?: number, height?: number): number {
    // ❌ НАРУШЕНИЕ OCP: Длинная цепочка if-else с проверкой типов
    // 💡 ПРОБЛЕМА: Нужно знать все возможные типы фигур
    // 🎯 РЕШЕНИЕ: Использовать полиморфизм - каждый объект сам знает, как вычислять периметр
    if (shapeType === 'circle') {
        if (!radius) throw new Error('Circle needs radius');
        ...
        // 💡 ПРОБЛЕМА: Дублирование проблемы - та же функция, те же изменения
        // 🎯 РЕШЕНИЕ: Triangle должен быть отдельным объектом с методом getPerimeter()
        if (!width) throw new Error('Triangle needs side');
        return 3 * width; // Упрощенно
        
    } else if (shapeType === 'square') {
        // ❌ НАРУШЕНИЕ OCP: И снова изменяем для квадрата!
        // 💡 ПРОБЛЕМА: Проблема повторяется в каждой функции
        // 🎯 РЕШЕНИЕ: Square должен быть отдельным объектом с методом getPerimeter()
        if (!width) throw new Error('Square needs side');
        return 4 * width;
    }
...
// 💡 ПРОБЛЕМА: Каждый раз нужно добавлять новые условия!
// 🎯 РЕШЕНИЕ: Работать с объектами, реализующими общий интерфейс
function processShape(shapeType: string, radius?: number, width?: number, height?: number): {area: number, perimeter: number} {
    ...
    // 💡 ПРОБЛЕМА: Эта функция зависит от других функций, которые тоже нарушают OCP
    // 🎯 РЕШЕНИЕ: Работать с объектами, имеющими методы getArea() и getPerimeter()
...
// 💡 ПРОБЛЕМА: Нужно тестировать всю функцию при добавлении новой фигуры
// 🎯 РЕШЕНИЕ: Использовать интерфейсы и полиморфизм
function demonstrateTestingProblems() {
    // ❌ ПРОБЛЕМА: Нельзя легко протестировать отдельные компоненты
    // const mockShape = { getArea: () => 10, getPerimeter: () => 20 };
    // processShape(mockShape); // Невозможно - функция ожидает строку и параметры
}
