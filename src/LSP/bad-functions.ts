...
// 💡 ПРЕИМУЩЕСТВО: Четкий контракт для обработки прямоугольников
...
// 💡 ПРОБЛЕМА: Нарушает контракт базовой функции
...
// 💡 ПРОБЛЕМА: Функция может получить неожиданное поведение от функций-оберток
...
function processShapes(processor: typeof processRectangle, shapes: Array<{width: number, height: number}>): {
    ...
    shapes.forEach((shape, index) => {
        try {
            ...
            // 💡 ПРОБЛЕМА: Ожидается: area = width * height (как в processRectangle)
            ...
            // 💡 ПРОБЛЕМА: Ожидается: perimeter = 2 * (width + height) (как в processRectangle)
            ...
            // 💡 ПРОБЛЕМА: Ожидается: "Rectangle: widthxheight" (как в processRectangle)
            ...
            // 💡 ПРОБЛЕМА: Ожидается: функция всегда работает без исключений (как processRectangle)
...
// 💡 ПРОБЛЕМА: Функции-обертки не могут заменить базовую функцию
...
// 💡 ПРОБЛЕМА: Ожидается: area = width * height, perimeter = 2 * (width + height)
...
// 💡 ПРОБЛЕМА: Ожидается: всегда работает без исключений
