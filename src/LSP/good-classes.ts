...
//
// 1. ✅ СОБЛЮДЕНИЕ КОНТРАКТА - классы-наследники выполняют все обещания родительского класса
// 2. ✅ ПРЕДСКАЗУЕМОЕ ПОВЕДЕНИЕ - код работает одинаково с любым классом-наследником
// 3. ✅ ИСТИННАЯ ПОДСТАНОВКА - дочерние классы могут корректно заменить родительский класс
...
class Rectangle implements Shape {
  ...
  getInfo(): `${string}: Area=${string}, Perimeter=${string}` {
    return `Rectangle: Area=${this.getArea().toFixed(
      2
...
class Circle implements Shape {
  ...
  // 🎯 РЕЗУЛЬТАТ: Предсказуемое поведение для клиентов
  getInfo(): `${string}: Area=${string}, Perimeter=${string}` {
    return `Circle: Area=${this.getArea().toFixed(
      2
    )}, Perimeter=${this.getPerimeter().toFixed(2)}`;
  }
...
class Triangle implements Shape {
  ...
  // 🎯 РЕЗУЛЬТАТ: Предсказуемое поведение для клиентов
  getInfo(): `${string}: Area=${string}, Perimeter=${string}` {
    return `Triangle: Area=${this.getArea().toFixed(
      2
    )}, Perimeter=${this.getPerimeter().toFixed(2)}`;
  }
...
function processShapes(shapes: Shape[]): {
  summaryArea: number;
  summaryPerimeter: number;
  summaryInfo: InfoType[];
} {
  let summaryArea = 0;
